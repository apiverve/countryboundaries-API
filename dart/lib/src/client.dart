...
class CountryboundariesValidationException implements Exception {
  final List<String> errors;

  CountryboundariesValidationException(this.errors);

  @override
  String toString() => 'CountryboundariesValidationException: ${errors.join("; ")}';
}
...
/// Country Boundaries API client.
///
/// For more information, visit: https://apiverve.com/marketplace/countryboundaries?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [country] (required) - The Country 2 Letter ISO code (e.g., CA) [minLength: 2, maxLength: 2]
class CountryboundariesClient {
  final String apiKey;
  ...
  CountryboundariesClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/countryboundaries',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CountryboundariesValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CountryboundariesValidationException(errors);
    }
  }

  /// Execute a request to the Country Boundaries API.
  ///
  /// Parameters are validated before sending the request.
  Future<CountryboundariesResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CountryboundariesException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CountryboundariesResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CountryboundariesException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CountryboundariesException('Resource not found');
      } else {
        throw CountryboundariesException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CountryboundariesException) rethrow;
      throw CountryboundariesException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Country Boundaries API client.
class CountryboundariesException implements Exception {
  final String message;

  CountryboundariesException(this.message);

  @override
  String toString() => 'CountryboundariesException: $message';
}
